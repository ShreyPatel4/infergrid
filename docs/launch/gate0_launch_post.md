...
![Quiet-tenant TTFT under noisy-neighbor contention — three arms compared on log scale, with the per-window trace showing zero warmup transients in the token-bucket arm](figures/launch_hero_chart.png)

*Top panel:* quiet-tenant TTFT p99 across three arms (log scale). Arm 0 solo baseline 54.9 ms; Arm 1 vanilla vLLM under flooder 28,716 ms (a 523× starvation); Arm 5b InferGrid with token-bucket rate-limit 74.2 ms. Net: a 387× reduction from Arm 1, leaving the quiet tenant within 1.35× of its solo baseline.

*Bottom panel:* per-window quiet TTFT p50 over the 120-second bench, comparing the sliding-window rate-limit (Arm 5, red — five transient windows above 1,000 ms) against the token-bucket rate-limit (Arm 5b, blue — every window at or near baseline). This panel is the rebuttal to "you hid a transient in the average." Generated by `scripts/generate_launch_chart.py` from the raw CSVs in `results/gate2_fairness_20260419/`.
